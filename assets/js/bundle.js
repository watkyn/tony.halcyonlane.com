...
Use 'ls' to look around.`})},ls:{description:"List directory contents",execute:(t,e)=>e.currentDirectory==="~"?{output:"about.txt  blog/  experience.txt  contact.txt"}:{output:"No files here."}},cat:{description:"Print file contents",execute:t=>{const e=t[0];if(!e)return{error:"cat: missing file operand"};if(e.endsWith("/"))return{error:`cat: ${e}: Is a directory`};const n={"about.txt":c.about.execute(),"experience.txt":c.experience.execute(),"contact.txt":c.contact.execute()};return e in n?n[e]:{error:`cat: ${e}: No such file`}}},pwd:{description:"Print working directory",execute:(t,e)=>({output:e.currentDirectory})},cd:{description:"Change directory",execute:(t,e)=>{const n=t[0];return!n||n==="~"?{output:"",state:{currentDirectory:"~"}}:n==="blog"?{output:"",state:{currentDirectory:`~/${n}`}}:{error:`cd: no such directory: ${n}`}}},clear:{description:"Clear the terminal",execute:()=>({clear:!0})}},p=Object.keys(c);function f(t,e){const{command:n,args:r}=d(t);if(!n)return{output:""};if(c[n]){const i=c[n].execute(r,e);return i.state?{...i,state:{...e,...i.state}}:i}return{error:`Command not found: ${n}. Type 'help' for available commands.`}}function y(t){const e=t.split(/\s+/).pop();return p.filter(n=>n.startsWith(e))}const g=`
 _____
|_   _|__  _ __  _   _
  | |/ _ \\| '_ \\| | | |
  | | (_) | | | | |_| |
...
|_____|_|\\___|_| |_|\\___|_|_.__/ \\___|_|  \\__, |\\___|_|
                                          |___/ 
`,h="Type 'help' to get started.";let o={currentDirectory:"~",commandHistory:[],historyIndex:-1};function u(t,e,n=0){return new Promise(r=>{let i=0;t.textContent="";function s(){i<e.length?(t.textContent+=e.charAt(i),i++,setTimeout(s,n)):r()}s()})}function a(t){const e=document.getElementById("terminal-content"),n=document.createElement("div");n.className="output",n.innerHTML=t,e.appendChild(n)}function x(t){const e=document.getElementById("terminal-content"),n=document.createElement("div");n.className="output-command",n.textContent=`guest@tony: ${t}`,e.appendChild(n)}function C(){const t=document.getElementById("terminal-content");t.innerHTML=""}function E(){const t=document.getElementById("terminal");t.scrollTop=t.scrollHeight}function l(){return`guest@tony:${o.currentDirectory}$`}function I(t){const e=t.trim();if(!e)return;x(e),o.commandHistory.push(e),o.historyIndex=o.commandHistory.length;const{runCommand:n}=globalThis.terminalEngine,r=n(t,o);r.clear?C():r.output?a(r.output):r.error&&a(`<span class="error">${r.error}</span>`),r.state&&(o={...o,...r.state}),E(),b()}function v(t){if(t.key==="Enter"){const e=t.target,n=e.value,r=e.closest(".command-line");I(n),r.remove(),m()}else if(t.key==="ArrowUp")t.preventDefault(),o.historyIndex>0&&(o.historyIndex--,t.target.value=o.commandHistory[o.historyIndex]);else if(t.key==="ArrowDown")t.preventDefault(),o.historyIndex<o.commandHistory.length-1?(o.historyIndex++,t.target.value=o.commandHistory[o.historyIndex]):(o.historyIndex=o.commandHistory.length,t.target.value="");else if(t.key==="Tab"){t.preventDefault();const e=t.target,n=e.value,r=n.split(/\s+/).pop(),{getTabCompletions:i}=globalThis.terminalEngine,s=i(r);if(s.length===1){const T=n.slice(0,n.length-r.length)+s[0];e.value=T}else s.length>1&&a(n+`
`+s.join("  "))}}function b(){document.querySelectorAll(".prompt").forEach(e=>e.textContent=l())}async function w(){const t=document.getElementById("terminal-content"),e=document.createElement("div");e.className="ascii-art",t.appendChild(e),await u(e,g);const n=document.createElement("div");n.className="welcome-message",t.appendChild(n),await u(n,h),m(),document.getElementById("command-input").focus()}function m(){const t=document.getElementById("terminal-content"),e=document.createElement("div");e.className="command-line",e.innerHTML=`
