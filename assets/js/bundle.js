...
|_____|_|\\___|_| |_|\\___|_|_.__/ \\___|_|  \\__, |\\___|_|
                                          |___/ 
`,h="Type 'help' to get started.";let o={currentDirectory:"~",commandHistory:[],historyIndex:-1};function l(t,n,e=-10){return new Promise(r=>{let i=0;t.textContent="";function s(){i<n.length?(t.textContent+=n.charAt(i),i++,setTimeout(s,e)):r()}s()})}function a(t){const n=document.getElementById("terminal-content"),e=document.createElement("div");e.className="output",e.innerHTML=t,n.appendChild(e)}function x(t){const n=document.getElementById("terminal-content"),e=document.createElement("div");e.className="output-command",e.textContent=`guest@tony: ${t}`,n.appendChild(e)}function C(){const t=document.getElementById("terminal-content");t.innerHTML=""}function E(){const t=document.getElementById("terminal");t.scrollTop=t.scrollHeight}function u(){return`guest@tony:${o.currentDirectory}$`}function v(t){const n=t.trim();if(!n)return;x(n),o.commandHistory.push(n),o.historyIndex=o.commandHistory.length;const{runCommand:e}=globalThis.terminalEngine,r=e(t,o);r.clear?C():r.output?a(r.output):r.error&&a(`<span class="error">${r.error}</span>`),r.state&&(o={...o,...r.state}),E(),w()}function I(t){if(t.key==="Enter"){const n=t.target,e=n.value,r=n.closest(".command-line");v(e),r.remove(),m()}else if(t.key==="ArrowUp")t.preventDefault(),o.historyIndex>0&&(o.historyIndex--,t.target.value=o.commandHistory[o.historyIndex]);else if(t.key==="ArrowDown")t.preventDefault(),o.historyIndex<o.commandHistory.length-1?(o.historyIndex++,t.target.value=o.commandHistory[o.historyIndex]):(o.historyIndex=o.commandHistory.length,t.target.value="");else if(t.key==="Tab"){t.preventDefault();const n=t.target,e=n.value,r=e.split(/\s+/).pop(),{getTabCompletions:i}=globalThis.terminalEngine,s=i(r);if(s.length===1){const T=e.slice(0,e.length-r.length)+s[0];n.value=T}else s.length>1&&a(e+`
`+s.join("  "))}}function w(){document.querySelectorAll(".prompt").forEach(n=>n.textContent=u())}async function b(){const t=document.getElementById("terminal-content"),n=document.createElement("div");n.className="ascii-art",t.appendChild(n),await l(n,g);const e=document.createElement("div");e.className="welcome-message",t.appendChild(e),await l(e,h),m(),document.getElementById("command-input").focus()}function m(){const t=document.getElementById("terminal-content"),n=document.createElement("div");n.className="command-line",n.innerHTML=`
    <span class="prompt">${u()}</span>
    <div class="input-area">
      <input type="text" id="command-input" autocomplete="off" spellcheck="false" autofocus>
    </div>
  `,t.appendChild(n);const e=document.getElementById("command-input");e.addEventListener("keydown",I),e.focus()}document.addEventListener("click",()=>{var t;(t=document.getElementById("command-input"))==null||t.focus()}),document.addEventListener("DOMContentLoaded",b),globalThis.terminalEngine={runCommand:y,createState:_,getTabCompletions:f}})();
